...
# ─── SERVIDOR WEB MÍNIMO (necesario para Render) ───
...
def buscar_productos(query):
    ...
    resultados = []
    for p in productos:
        haystack = normalizar(f"{p['nombre']} {p['categoria']} {p['keywords']}")
        if all(t in haystack for t in terminos):
            resultados.append(p)
    return resultados
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/categorias — Ver productos por categoría\n"
...
async def categorias(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cats:
        await update.message.reply_text("⚠️ No hay categorías disponibles. Verifica el catálogo.")
        return
...
async def buscar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(resultados) == 1:
        await update.message.reply_text(
            formatear_producto(resultados[0]),
            parse_mode="Markdown"
        )
        return
...
if __name__ == "__main__":
    ...
    # Arrancar servidor web en hilo separado para Render
    hilo = threading.Thread(target=iniciar_servidor, daemon=True)
    hilo.start()
    logging.info(f"Servidor web activo en puerto {PORT}")

    # Verificar token
    logging.info(f"TOKEN recibido: {'SI' if TOKEN else 'NO - TOKEN ES NONE'}")
    if not TOKEN:
        logging.error("ERROR CRITICO: BOT_TOKEN no encontrado en variables de entorno")
        raise ValueError("BOT_TOKEN no configurado")

    async def main():
        try:
            logging.info("Construyendo aplicación Telegram...")
